...
Setting the position of a client allows you to simply move the cursor from one computer to another. To set a clients position, we use the set command. The set command allows you to assign client-A to be at an edge of client-B. For example, if you have a laptop on your desk that is to the left of your desktop computer, you would use the set command like so:
...
The input client can also be switched via a hotkey. A hotkey can be a normal key with three possible modifiers; shift, control and alt.
